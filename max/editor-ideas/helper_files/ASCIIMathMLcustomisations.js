// some changes made by Max Starkenburg
...
function AMdisplay(inputNode, outputNode, now) {
    if(!now){
        if(!key1 && !key2) {
            key1 = true;
//            setTimeout("AMdisplay('"+inputNodeId+"', '"+outputNodeId+"', true);", 250);
            setTimeout("AMdisplay('"+inputNode+"', '"+outputNode+"', true);", 250);
        } else if (key1 && !key2) {
            key2 = true;
        }
    }
...
*/
            if (inputNode != null) {
                var str = "`" + inputNode.text() + "`";
//                var outnode = document.getElementById(outputNodeId);
/*
                var newnode = AMcreateElementXHTML("span");
                newnode.setAttribute("id",outputNode.attr("id"));
                //JG changed the line below from setAttribute("class","answer") because it didn't work with IE
                newnode.className = "answer";
                outputNode.replaceWith(newnode);
                outputNode2 = document.getElementById(outputNode.attr("id"));
//alert($(outputNode2).attr("id"));
                var n = outputNode2.childNodes.length;
                for (var i = 0; i < n; i++)
                    outputNode2.removeChild(outputNode2.firstChild);
                var arr = str.split("#");
                var frag = AMarr2docFrag(arr);
                outputNode2.appendChild(frag);
                AMprocessNode(outputNode2,true);
*/
...
//                outputNode2 = document.getElementById(outputNode.attr("id"));
/*
                var n = outputNode.get(0).children().length;
                for (var i = 0; i < n; i++)
                    outputNode.get(0).removeChild(outputNode.firstChild);
*/
//                outputNode.children().remove();
                outputNode.empty();
                var arr = str.split("#");
                var frag = AMarr2docFrag(arr);
                outputNode.append(frag);
                AMprocessNode(outputNode.get(0),true);
                // MS: I don't know why it's adding a second row of empty <math> ... so hackily getting rid of it
                outputNode.children("math").eq(1).remove();
                // MS: repopulate the .ascii-notation ... I don't want to store this in the title attribute
                outputNode.siblings(".ascii-notation").html(inputNode.text());
            }
        }
    }
}
...
// MS: just to remove the title attribute, i think
function AMparseMath2(str) {
...
*/
//  $(node).before('<span class="ascii-notation">' + str.replace(/\s+/g," ") + '</span>');
//alert($(node).siblings(".ascii-notation").length);
//  $(node).siblings('.ascii-notation').html(str.replace(/\s+/g," "));
